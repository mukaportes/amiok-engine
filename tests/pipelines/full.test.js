...
jest.mock('../../src/process/clinic-doctor/start', () =>
  jest.fn().mockResolvedValue({ key: 'DOCTOR_START', val: 1 })
);
jest.mock('../../src/process/settings/prepare', () =>
  jest.fn().mockResolvedValue({ key: 'SETTINGS_PREPARE', val: 1 })
);
jest.mock('../../src/process/test-script/execute', () =>
  jest.fn().mockResolvedValue({ key: 'SCRIPT_EXECUTE', val: 1 })
);
...
jest.mock('../../src/process/clinic-doctor/clear', () =>
  jest.fn().mockResolvedValue({ key: 'DOCTOR_CLEAR', val: 1 })
);
jest.mock('../../src/process/clinic-doctor/shutdown', () =>
...
jest.mock('../../src/process/info/api-pid', () =>
  jest.fn().mockResolvedValue({ key: 'INFO_API_PID', val: 1 })
);
...
jest.mock('../../src/process/storage/setup-test', () =>
  jest.fn(
    (params) =>
      new Promise((resolve, reject) => {
...
jest.mock('../../src/process/storage/store-test', () =>
  jest.fn().mockResolvedValue({ key: 'STORAGE_TEST', val: 1 })
);
